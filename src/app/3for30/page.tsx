...
export const metadata: Metadata = {
  title: "3 for £30 Christmas Gifts | Holly Jolly Savings",
};
...
async function loadProductsForGroup(name: string) {
  try {
    ...
    console.warn("3-for-30 bundle: Prisma unavailable, returning empty group", error);
    return [];
  }
}

export default async function ThreeFor30Page() {
  const region = getCurrentRegion();
  const isUK = region.id === "uk";
  const products = await loadProductsForGroup("3-for-30");
    ...
    <Layout>
      <section className="space-y-6 rounded-3xl border border-[#D9A441]/60 bg-black/85 p-5 shadow-lg shadow-black/70 sm:p-8 lg:p-10">
        <h1 className="text-3xl font-bold text-[#FFF9F2] sm:text-4xl">
          3 for {isUK ? "£30" : "$35"} Gifts
        </h1>
        <p className="text-sm text-[#E5E7EB]">
          Premium gifts that qualify for our 3-for-
          {isUK ? "£30" : "$35"} bundle. Build a higher-value bundle while still

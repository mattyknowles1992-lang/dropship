...
export const metadata: Metadata = {
  title: "3 for £50 Christmas Gifts | Holly Jolly Savings",
};
...
async function loadProductsForGroup(name: string) {
  try {
    ...
    console.warn("3-for-50 bundle: Prisma unavailable, returning empty group", error);
    return [];
  }
}

export default async function ThreeFor50Page() {
  const region = getCurrentRegion();
  const isUK = region.id === "uk";
  const products = await loadProductsForGroup("3-for-50");
    ...
    <Layout>
      <section className="space-y-6 rounded-3xl border border-[#D9A441]/60 bg-black/85 p-5 shadow-lg shadow-black/70 sm:p-8 lg:p-10">
        <h1 className="text-3xl font-bold text-[#FFF9F2] sm:text-4xl">
          3 for {isUK ? "£50" : "$60"} Gifts
        </h1>
        <p className="text-sm text-[#E5E7EB]">
          Higher-ticket gifts that still work within a simple 3-for bundle.
          Perfect for premium stockings, special family members or close
          friends.
        </p>

...
export function canonicalizeSupplier(value: string | null | undefined): string | null {
  const trimmed = value?.trim() ?? "";
  if (!trimmed) return null;
  const lower = trimmed.toLowerCase();
  if (["cj", "cjdropshipping", "cj dropshipping"].includes(lower)) return "cjdropshipping";
  if (["aliexpress", "ali express", "ali"].includes(lower)) return "aliexpress";
  return trimmed;
}
